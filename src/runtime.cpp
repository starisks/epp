...
class Interpreter {
 ...
 private:
  ...
  [[noreturn]] void throwErr(const std::string& msg, Span sp) { throw RuntimeError{msg, sp}; }
  ...
  static bool valuesEqual(const Value& a, const Value& b) {
    ...
    if (isNumber(a)) return std::get<double>(a.data) == std::get<double>(b.data);
    if (isString(a)) return std::get<std::string>(a.data) == std::get<std::string>(b.data);
    if (isBool(a)) return std::get<bool>(a.data) == std::get<bool>(b.data);
    if (isArray(a)) return std::get<Value::Array>(a.data) == std::get<Value::Array>(b.data);
  ...
  Value evalBinary(const BinaryExpr& be, const EnvPtr& env) {
    ...
    switch (be.op) {
      case BinaryOp::Add: {
        ...
        throwErr("Operator '+' expects numbers or strings", be.span);
      }
      case BinaryOp::Sub: {
        if (!isNumber(l) || !isNumber(r)) throwErr("Operator '-' expects numbers", be.span);
      ...
      case BinaryOp::Eq: return Value::boolean(valuesEqual(l, r));
      case BinaryOp::Neq: return Value::boolean(!valuesEqual(l, r));
      case BinaryOp::Gt:
      case BinaryOp::Lt:
      case BinaryOp::Gte:
      case BinaryOp::Lte: {
        if (!isNumber(l) || !isNumber(r)) throwErr("Comparison expects numbers", be.span);
        double a = std::get<double>(l.data);
        double b = std::get<double>(r.data);
        bool ok = false;
        if (be.op == BinaryOp::Gt) ok = a > b;
        else if (be.op == BinaryOp::Lt) ok = a < b;
        else if (be.op == BinaryOp::Gte) ok = a >= b;
        else ok = a <= b;
