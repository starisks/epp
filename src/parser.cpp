...
class Parser {
 ...
 private:
  ...
  Token expect(TokenType t, std::vector<ParseError>& errors, const std::string& msg) {
    ...
    errors.push_back(ParseError{msg, peek().span});
  ...
  ExprPtr conditionExpr(std::vector<ParseError>& errors) {
    // Parse comparisons in English:
    // <expr> is (not)? (greater|less|equal) (than)? (to)? <expr>
    // If no "is ..." pattern, falls back to expression.
    ...
    if (match(TokenType::Greater)) {
      ...
      auto op = isNot ? BinaryOp::Lte : BinaryOp::Gt;
      if (!right) right = std::make_unique<NumberExpr>(0.0, peek().span);
      return std::make_unique<BinaryExpr>(op, std::move(left), std::move(right), left->span);
    }

    if (match(TokenType::Less)) {
      match(TokenType::Than);
      auto right = expression(errors);
      auto op = isNot ? BinaryOp::Gte : BinaryOp::Lt;
      if (!right) right = std::make_unique<NumberExpr>(0.0, peek().span);
      return std::make_unique<BinaryExpr>(op, std::move(left), std::move(right), left->span);
    }
    ...
    if (match(TokenType::Equal)) {
      ...
      auto right = expression(errors);
      auto op = isNot ? BinaryOp::Neq : BinaryOp::Eq;
      if (!right) right = std::make_unique<NumberExpr>(0.0, peek().span);
      return std::make_unique<BinaryExpr>(op, std::move(left), std::move(right), left->span);
    }

    // also allow: "is not <expr>" meaning !=
    if (isNot) {
      auto right = expression(errors);
      if (!right) right = std::make_unique<NumberExpr>(0.0, peek().span);
      return std::make_unique<BinaryExpr>(BinaryOp::Neq, std::move(left), std::move(right),
                                          left->span);
    }
